#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_deb.sh — construit le paquet Debian du module Kubuno Books.
#
# Produit le MÊME layout d'installation que le monorepo, afin que le core
# découvre le module à l'identique :
#   /usr/lib/kubuno/modules/books/{kubuno-books, module.toml, frontend/}
#   /usr/share/kubuno/modules/books/migrations/*.sql
#   /etc/kubuno/modules/books/config.toml.example
#
# Usage :
...
Depends: libssl3, ca-certificates, kubuno-core
Section: web
Priority: optional
Homepage: https://github.com/kubuno/${MODULE}
Description: Kubuno Books — bibliothèque de livres, BD, comics et eBooks (build ${BUILD_NUM})
EOF
